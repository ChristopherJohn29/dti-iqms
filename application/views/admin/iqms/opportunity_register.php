...
$(function(){
  ...
  var moduleCode='OPPORTUNITY_REGISTER', officeId=10, processId=1, fiscalYear='2025';
  var analysisId=null;
  ...
  function loadOpportunities(){
    return $.getJSON(ENDPOINT.LIST,{table:'iqms_opportunity_register',analysis_id:analysisId},function(rows){ renderOpportunitiesTableDb(rows); populateOpportunityDropdownsDb(rows); loadMonitoringTable(rows); });
  }

  function computeScore(prob, si, se, fi, ri){ var totalImpact = Math.round((parseInt(si)||0 + parseInt(se)||0 + parseInt(fi)||0 + parseInt(ri)||0)/4); return (parseInt(prob)||0) + totalImpact; }
  function priorityBadge(score){ return score>=5? '<span class="badge bg-danger">1st Priority</span>': score>=3? '<span class="badge bg-warning">2nd Priority</span>':'<span class="badge bg-success">3rd Priority</span>'; }

  function renderOpportunitiesTableDb(rows){
    ...
    $.each(rows,function(_,o){
      ...
      var row = $('<tr/>').html(
        '<td>'+(o.opportunity_id||('OR-'+o.id))+'</td>'+
        '<td>'+escape(o.opportunity_description||'')+'</td>'+
        '<td>'+(o.probability||'')+'</td>'+
        '<td>'+Math.round((o.overall_score||0))+'</td>'+
        '<td>'+score+'</td>'+
        '<td>'+priorityBadge(score)+'</td>'+
        '<td>'+
  ...
  function populateOpportunityDropdownsDb(rows){
    var $sel1=$('#treatmentOpportunityId').empty().append('<option value="">Select Opportunity</option>');
    var $sel2=$('#treatmentOpportunityFilter').empty().append('<option value="">All Opportunities</option>');
    $.each(rows,function(_,o){ $('<option/>',{value:o.id, text:o.opportunity_id||('OR-'+o.id)}).appendTo($sel1); $('<option/>',{value:o.id, text:o.opportunity_id||('OR-'+o.id)}).appendTo($sel2); });
  }

  window.openOpportunityModal = function(){ $('#opportunityModalTitle').text('Add New Opportunity'); $('#opportunityId').val(''); $('#opportunityForm')[0].reset(); $('#opportunityModal').show(); };
  window.closeOpportunityModal = function(){ $('#opportunityModal').hide(); };

  window.editOpportunity = function(id){ $('#opportunityModalTitle').text('Edit Opportunity'); $('#opportunityId').val(id);
    $.getJSON(ENDPOINT.LIST,{table:'iqms_opportunity_register',analysis_id:analysisId}, function(rows){ var o=rows.find(function(x){return x.id==id;}); if(!o) return; $('#opportunityDescription').val(o.opportunity_description||''); $('#opportunityProbability').val(o.probability||''); $('#strategicImpact').val(o.strategic_impact||''); $('#serviceImpact').val(o.service_impact||''); $('#financialImpact').val(o.financial_impact||''); $('#reputationImpact').val(o.reputation_impact||''); $('#actions').val((o.actions||'')); $('#responsibility').val(o.opportunity_owner||''); $('#timeline').val(o.target_implementation_date||''); $('#indicator').val(o.effectiveness_indicator||''); $('#opportunityModal').show(); }); };

  window.deleteOpportunity = function(id){ if(!confirm('Are you sure you want to delete this opportunity?')) return; $.post(ENDPOINT.DEL,{table:'iqms_opportunity_register',id:id},function(){ loadOpportunities(); }); };

  window.saveOpportunity = function(){ var id=$('#opportunityId').val()||null; var data={ table:'iqms_opportunity_register', id:id, analysis_id:analysisId,
  ...
  }; $.post(ENDPOINT.SAVE,data,function(){ alert('Opportunity saved successfully!'); closeOpportunityModal(); loadOpportunities(); }); };

  // -------- Treatments --------
  function loadTreatments(){ $.getJSON(ENDPOINT.LIST,{table:'iqms_opportunity_actions',analysis_id:analysisId}, function(rows){ renderTreatmentsTableDb(rows); }); }
  function renderTreatmentsTableDb(rows){ var $tbody=$('#treatmentTable tbody').empty(); $.each(rows,function(_,t){ var row='<tr>'+
      '<td>'+(t.opportunity_id?('OR-'+t.opportunity_id):'')+'</td>'+
      '<td>'+escape((t.treatment_type||'').toLowerCase())+'</td>'+
      '<td>'+escape(t.action_description||'')+'</td>'+
      '<td>'+escape(t.responsibility||'')+'</td>'+
      '<td>'+formatDate(t.target_date)+'</td>'+
      '<td>'+formatDate(t.completion_date)+'</td>'+
      '<td>'+(t.progress||'')+'%</td>'+
      '<td>'+escape(t.action_status||'')+'</td>'+
      '<td><button class="btn btn-warning btn-sm" onclick="editTreatment('+(t.id)+')"><i class="fe-edit"></i></button></td>'+
    '</tr>'; $tbody.append(row); }); }

  window.openTreatmentModal = function(){ $('#treatmentModalTitle').text('Add Treatment Plan'); $('#treatmentId').val(''); $('#treatmentForm')[0].reset(); $('#treatmentModal').show(); };
  window.closeTreatmentModal = function(){ $('#treatmentModal').hide(); };

  window.editTreatment = function(id){ $('#treatmentModalTitle').text('Edit Treatment Plan'); $('#treatmentId').val(id);
    $.getJSON(ENDPOINT.LIST,{table:'iqms_opportunity_actions',analysis_id:analysisId},function(rows){ var t=rows.find(function(x){return x.id==id;}); if(!t)return; $('#treatmentOpportunityId').val(t.opportunity_id); $('#treatmentAction').val((t.treatment_type||'').toLowerCase()); $('#treatmentDescription').val(t.action_description||''); $('#treatmentResponsible').val(t.responsibility||''); $('#treatmentProgress').val(t.progress||0); $('#treatmentStartDate').val(t.target_date||''); $('#treatmentDueDate').val(t.completion_date||''); $('#treatmentStatus').val((t.action_status||'').toLowerCase()); $('#treatmentModal').show(); }); };

  window.saveTreatment = function(){ var id=$('#treatmentId').val()||null; var data={ table:'iqms_opportunity_actions', id:id,
  ...
  }; $.post(ENDPOINT.SAVE,data,function(){ alert('Treatment plan saved successfully!'); closeTreatmentModal(); loadTreatments(); }); };

  // -------- Monitoring --------
  function loadMonitoringTable(opportunities){ var $tbody=$('#monitoringTable tbody').empty(); $.each(opportunities,function(_,o){
    $.getJSON(ENDPOINT.CHILDREN,{table:'iqms_opportunity_monitoring',fk:'opportunity_id',id:o.id}, function(ms){ var q={Q1:'N',Q2:'N',Q3:'N',Q4:'N'}; $.each(ms,function(_,m){ q[m.quarter]=m.monitoring_result; }); var row='<tr>'+
      '<td class="text-center">'+(o.opportunity_id||('OR-'+o.id))+'</td>'+
      '<td>'+escape(o.effectiveness_indicator||'')+'</td>'+
  ...
  window.openMonitoringModal = function(oppId){ var $m=$('#monitoringModal'); $('#monitoringForm')[0].reset(); $('#monitoringOpportunityId').val(oppId); $.getJSON(ENDPOINT.CHILDREN,{table:'iqms_opportunity_monitoring',fk:'opportunity_id',id:oppId}, function(ms){ var map={}; $.each(ms,function(_,m){ map[m.quarter]=m; }); $('#monitoringIndicator').val(''); // indicator comes from register row if needed
      $('#q1Status').val(map.Q1? map.Q1.monitoring_result:'N'); $('#q2Status').val(map.Q2? map.Q2.monitoring_result:'N'); $('#q3Status').val(map.Q3? map.Q3.monitoring_result:'N'); $('#q4Status').val(map.Q4? map.Q4.monitoring_result:'N'); $m.show(); }); };
  window.closeMonitoringModal = function(){ $('#monitoringModal').hide(); };

  window.saveMonitoringStatus = function(){ var oppId=$('#monitoringOpportunityId').val(); var quarters=['Q1','Q2','Q3','Q4']; var i=0; (function next(){ if(i>=quarters.length){ alert('Monitoring status updated successfully!'); closeMonitoringModal(); return ensureAnalysis().then(loadOpportunities); } var q=quarters[i++]; var val=$('#'+q.toLowerCase()+'Status').val(); $.getJSON(ENDPOINT.CHILDREN,{table:'iqms_opportunity_monitoring',fk:'opportunity_id',id:oppId,extra_key:'quarter',extra_val:q}, function(rows){ var payload={table:'iqms_opportunity_monitoring', opportunity_id:oppId, quarter:q, monitoring_result:val}; if(rows.length){ payload.id=rows[0].id; } $.post(ENDPOINT.SAVE, payload, function(){ next(); }); }); })(); };
  ...
  // Initialize
  ensureAnalysis().then(loadOpportunities);
});
    {
        id: "OR-1",
